...
import { Download, Link as LinkIcon, Zap, Check, AlertCircle, Search, Film, Music, HardDrive, Copy, ExternalLink, Play, X } from 'lucide-react';
import { useLang, useToast, Confetti } from '../App';
...
function SkeletonLoader() {
  return (
    <div style={{ marginTop: '1.5rem', animation: 'fade-in 0.3s ease both' }}>
      ...
      <div className="skeleton-quality-grid">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="skeleton skeleton-quality-item"></div>
        ))}
      </div>
    </div>
  );
...
/* ═══════ VIDEO PREVIEW ═══════ */
function VideoPreview({ thumbnail, url }) {
  const [showModal, setShowModal] = useState(false);
  if (!thumbnail) return null;

  return (
    <>
      <div className="video-preview-container" onClick={() => setShowModal(true)}>
        <img src={thumbnail} alt="thumbnail" onError={(e) => e.target.style.display = 'none'} />
        <div className="video-preview-play">
          <Play size={18} />
        </div>
      </div>
      {showModal && (
        <div className="video-preview-modal" onClick={() => setShowModal(false)}>
          <button className="video-preview-close" onClick={() => setShowModal(false)}>
            <X size={20} />
          </button>
          <img
            src={thumbnail}
            alt="Preview"
            style={{ maxWidth: '90vw', maxHeight: '80vh', borderRadius: '20px', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}
...
/* ═══════ MOBILE BOTTOM SHEET ═══════ */
function BottomSheet({ formats, selectedFormat, onSelect, onClose, t }) {
  return (
    <>
      ...
      <div className="bottom-sheet">
        ...
        <div className="bottom-sheet-title">
          <HardDrive size={16} /> {t.download.selectQuality}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '0.6rem' }}>
          {formats.map((fmt) => (
            <button
              key={fmt.formatId}
              type="button"
              onClick={() => { onSelect(fmt.formatId); onClose(); }}
              className="clay-card"
              style={{
...
export default function Home() {
  ...
  const handleFetchInfo = async (e) => {
    ...
    try {
      ...
      addToast(t.toast.downloadStarted || 'Video info fetched!', 'success');
    } catch (err) {
      setError(err.response?.data?.message || t.download.fetchError);
  ...
  const handleDownload = () => {
    ...
    setResult(null);
    setError(null);
    setProgress({ percent: 0, status: 'downloading', message: t.download.connecting });
    addToast(t.toast.downloadStarted, 'info');
    ...
    es.onmessage = (event) => {
      try {
        ...
        if (data.status === 'done') {
          setProgress({ percent: 100, status: 'done', message: t.download.complete });
          setResult({ success: true, message: t.download.complete, fileUrl: data.fileUrl });
          ...
          setProgress(null);
          setResult({ success: false, message: data.message || t.download.failed });
          setDownloading(false);
          addToast(t.toast.error, 'error');
          es.close();
        } else {
    ...
    es.onerror = () => {
      setProgress(null);
      setResult({ success: false, message: t.download.connectionLost });
      setDownloading(false);
      addToast(t.toast.error, 'error');
      es.close();
    };
  ...
  const handleCopyLink = (fileUrl) => {
    ...
    addToast(t.download.linkCopied || t.toast.copied, 'success');
  };

  const STEPS = [
    { num: 1, title: t.steps.step1.title, desc: t.steps.step1.desc },
    { num: 2, title: t.steps.step2.title, desc: t.steps.step2.desc },
    { num: 3, title: t.steps.step3.title, desc: t.steps.step3.desc },
  ];
  ...
  return (
    <div>
        ...
        <div className="clay-card clay-card-orange download-card">
          ...
          <form onSubmit={handleFetchInfo} className="download-form" id="download-form">
            <div className="download-input-row">
              ...
              >
                {fetching ? (
                  <><div className="clay-loader"></div> {t.download.fetching}</>
                ) : (
                  <><Search size={20} /> {t.download.fetchBtn}</>
                )}
              </button>
            </div>
          ...
          {videoInfo && (
            <div style={{ marginTop: '1.5rem', animation: 'pop-in 0.4s cubic-bezier(0.68,-0.55,0.27,1.55) both' }}>
              ...
              {/* Video Preview */}
              <div className="clay-card" style={{ display: 'flex', gap: '1.25rem', padding: '1.25rem', marginBottom: '1.25rem', alignItems: 'center', flexWrap: 'wrap' }}>
                <VideoPreview thumbnail={videoInfo.thumbnail} url={url} />
                <div style={{ flex: 1, minWidth: '200px' }}>
              ...
              {/* Quality Picker — Desktop grid / Mobile bottom sheet trigger */}
              {!downloading && !result && (
                <>
                  {isMobile ? (
                    <div style={{ marginBottom: '1.25rem' }}>
                      <button
                        type="button"
                        className="clay-btn clay-btn-secondary clay-btn-full"
                      ...
                      >
                        ...
                        <span style={{ fontSize: '0.85rem', color: 'var(--orange-500)', fontWeight: 700 }}>
                          {videoInfo.formats.find(f => f.formatId === selectedFormat)?.label || '—'}
                        </span>
                      </button>
                      {showBottomSheet && (
                        <BottomSheet
              ...
              {!downloading && !result && (
                ...
                >
                  <Download size={22} /> {t.download.downloadNow}
                </button>
              )}
                ...
                <div className="progress-container">
                  <div className="progress-header">
                    <span className="progress-status">
                      {progress.status === 'merging' ? `🔀 ${t.download.merging}` :
                       progress.status === 'downloading' ? `⬇️ ${t.download.downloading}` : `⏳ ${t.download.processing}`}
                    </span>
                    ...
                    <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                        ...
                        <button 
                          ...
                          onClick={() => handleCopyLink(result.fileUrl)}
                      ...
                      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                        <a href={result.fileUrl} target="_blank" rel="noopener noreferrer" className="clay-btn clay-btn-success" style={{ display: 'inline-flex', textDecoration: 'none' }}>
                          <ExternalLink size={18} /> {t.download.openVideo}
                        </a>
      ...
      <section className="sites-section">
        <h2 className="section-title">{t.sites.title}</h2>
      ...
      <section className="disclaimer-section">
        <div className="disclaimer-card">
          <p>
            <strong>{t.disclaimer.title}</strong><br />
            {t.disclaimer.text}
          </p>
        </div>
      </section>
